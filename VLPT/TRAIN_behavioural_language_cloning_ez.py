...
def VPT_train():
    ...
    for batch_i, (video_group_id, subseq_ids, batch_frames, batch_words, batch_actions, finished_videos) in enumerate(train_data_loader):
        ...
        print('action labels shape',action_labels['buttons'].shape)
        x_words[:] = action_labels['buttons'].view([4,128]).clone()
        y_words[:] = action_labels['buttons'].view([4,128]).clone()
...
def VPT_evaluate():
    ...
    with th.no_grad():
        ...
        for batch_i, (eval_video_group_id, eval_subseq_ids, eval_batch_frames, eval_batch_words, eval_batch_actions, _) in enumerate(eval_data_loader):
            ...
            x_words[:] = eval_action_labels['buttons'].view([4,128]).clone()
            y_words[:] = eval_action_labels['buttons'].view([4,128]).clone()
