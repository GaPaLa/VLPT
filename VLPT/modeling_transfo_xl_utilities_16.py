...
"""
 ...
 (modified further by GPPL for efficient loss calculation - THIS CHANGES HOW LABELS ARE EXPECTED TO BE PASSED, BREAKING THE LMHEADMODEL loss and predictions!!! Do not use it with this modified file!)
"""
...
class ProjectedAdaptiveLogSoftmax(nn.Module):
    def __init__(self, n_token, d_embed, d_proj, cutoffs, div_val=1, keep_order=False):
        print('W: CUSTOM TRANSFO_XL_UTILITIES.py label usage not like HF docs')
        super().__init__()
        ...
        if div_val == 1:
            for i in range(len(self.cutoffs)):
                if d_proj != d_embed:
                    self.out_projs.append(nn.Parameter(torch.BFloat16Tensor(d_proj, d_embed)))
                else:
            ...
            for i in range(len(self.cutoffs)):
                ...
                self.out_projs.append(nn.Parameter(torch.BFloat16Tensor(d_proj, d_emb_i)))
